...
# pip3 install langchain_community langchainhub chromadb langchain langgraph tavily-python langchain-text-splitters langchain_openai
...
class GraphState(TypedDict):
  ...
  searched: str
  regenerate: str
  hallucination_checker_result: str
  documents: List[str]
...
def hallucination_checker(state):
  ...
  if grade == "yes":
    print("---HALUCINATION YES---")
    state["hallucination_checker_result"] = "Yes"
  else:
    print("---HALUCINATION NO---")
    state["hallucination_checker_result"] = "No"

  return {
    "documents": documents,
    "question": question,
    "hallucination_checker_result": state["hallucination_checker_result"],
    "generation": generation
  }
...
def decide_to_regenerate(state):
  if state["regenerate"] == "Yes":
    print("failed: hallucination")
    return "failed"

  if state["hallucination_checker_result"] == "Yes":
    print("---DECISION: HALLUCINATION CHECKER RESULT YES ---")
    return "no"
  else:
    print("---DECISION: HALLUCINATION CHECKER RESULT NO ---")
    state["regenerate"] = "Yes"
    return "yes"
...
def decide_to_generate(state):
  ...
  if state["searched"] == "Yes" and state["web_search"] == "Yes":
...
workflow.add_node("hallucination_checker", hallucination_checker)  # hallucination checker
...
if "generation" in value:
  ...
  # metadata
  pprint(value["documents"][0].metadata)
else:
  pprint("---END: Failed to generate answer---")
  pprint(value)
